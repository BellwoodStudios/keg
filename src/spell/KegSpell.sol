...
// decimals & precision
    ...
    function execute() external limited {
        ...
        // Create Flap Tap
        KegAbstract FLAP_TAP_DISPATCHER = KegAbstract(address(new Keg(MCD_DAI)));
        address FLAP_TAP = address(new FlapTap(FLAP_TAP_DISPATCHER, DaiJoinAbstract(MCD_JOIN_DAI), MCD_FLAP, 20 * WAD / 100)); // 20%

        FlapTap(FLAP_TAP).rely(MCD_VOW);
        FlapAbstract(MCD_FLAP).deny(MCD_VOW);
        FlapAbstract(MCD_FLAP).rely(FLAP_TAP);

        CHANGELOG.setAddress("FLAP_TAP_DISPATCHER", address(FLAP_TAP_DISPATCHER));
        CHANGELOG.setAddress("FLAP_TAP", FLAP_TAP);

        // Create Tap
        KegAbstract TAP_DISPATCHER = KegAbstract(address(new Keg(MCD_DAI)));

        address TAP = address(new Tap(TAP_DISPATCHER, DaiJoinAbstract(MCD_JOIN_DAI), MCD_VOW, 400000*WAD/ 365 days));

        FlapTap(FLAP_TAP).rely(MCD_VAT);

        CHANGELOG.setAddress("TAP_DISPATCHER", address(TAP_DISPATCHER));
        CHANGELOG.setAddress("TAP", address(TAP));

        ////// seat Setting
        // TODO
        address[] memory wallets = new address[](1);
        wallets[0] = 0x0000000000000000000000000000000;
        uint256[] memory amts = new uint256[](1);
        amts[0] = 1 ether;   // 50% split
        TAP_DISPATCHER.seat(wallets, amts);
        //
        ...
        uint256[] memory amts = new uint256[](1);
        amts[0] = 1 ether;   // 50% split
        FLAP_TAP_DISPATCHER.seat(wallets, amts);
        //
